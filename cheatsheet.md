...
| `tblA.join(colA, tblB)` or `tblA.join(colA, tblB, colB)` | Generates a table with the columns of `tblA` and `tblB`, containing rows for all values in `colA` and `colB` that appear in `tblA` and `tblB`, respectively. | 1. **string**: name of  column in `tblA` <br> 2. **Table**: other table <br> 3. (Optional) **string**: name of shared column in `tblB` | **Table**: a new combined table |
